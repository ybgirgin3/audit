...
from .models import Question, Mark, Category, Audit, UserAuditProgress, UserResponse, AdminComment, Result
...
class Quiz(View):
   def get(self, request):
        ...
        if selected_category_id:
            questions = Question.objects.filter(category__id=selected_category_id, verified=True)
        else:
   ...
   def post(self, request):
        ...
        if wrong_answers:
            messages_str = "<br>".join([f"Soru: {item['question_id']}, Risk Seviyesi: {item['risk_level']}" for item in wrong_answers])
            messages.error(request, mark_safe(f"Yanlış cevaplanan sorular ve risk seviyeleri:<br>{messages_str}"))
        else:
...
class AuditView(View):
    def get(self, request):
        ...
        user_progress = UserAuditProgress.objects.filter(user=request.user).select_related('audit')
        progress_dict = {progress.audit.id: progress for progress in user_progress}

        for audit in audits:
            audit.progress = progress_dict.get(audit.id)

        return render(request, "quiz/audits_list.html", {
            "audits": audits,
        })
